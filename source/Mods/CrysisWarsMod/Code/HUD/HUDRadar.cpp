...
void CHUDRadar::Update(float fDeltaTime)
{
	...
	if(isMultiplayer)
	{
		...
		for(int i = 0; i < locations.size(); ++i)
		{
			...
			if (pEntity)
			{
				...
				int friendly = FriendOrFoe(isMultiplayer, playerTeam, pEntity, pGameRules);
				if(friendly==2)
				{
				...
				fX = m_fX + vTransformed.x;
				fY = m_fY - vTransformed.y;
				...
				float dimX = (m_fX + fRadarSizeOverTwo) - lowerBoundX;
				float dimY = (m_fY + fRadarSizeOverTwo) - lowerBoundY;
				numOfValues += ::FillUpDoubleArray(&entityValues, pEntity->GetId(), ESpawnPoint /*building entity*/, (fX - lowerBoundX) / dimX, (fY - lowerBoundY) / dimY, 0.0f, friendly, sizeScale*100.0f, 100.0f);
			}
		}
	
		for(int t = 0; t < m_buildingsOnRadar.size(); ++t)
		{
			EntityId id = m_buildingsOnRadar[t].m_id;
			if(id)
			{
				IEntity *pEntity = gEnv->pEntitySystem->GetEntity(id);
				...
				if(pEntity)
				{
					...
					if(type==EHeadquarter || type==EHeadquarter2)
						continue;

					numOfValues += ::FillUpDoubleArray(&entityValues, pEntity->GetId(), (int)type, (fX - lowerBoundX) / dimX, (fY - lowerBoundY) / dimY, 0.0f, FriendOrFoe(gEnv->bMultiplayer, playerTeam, pEntity, g_pGame->GetGameRules()), sizeScale*100.0f, 100.0f);
				}
			}
		}
	}
	...
	if(pActor)
	{
		...
		SFlashVarValue args[3] = {playerX, playerY, 180.0f-GetRadarRotation(pActor)};
...
float CHUDRadar::GetRadarRotation(CActor *actor)
{
	if(!actor)
		return 0.0f;
	...
	IVehicle *vehicle = actor->GetLinkedVehicle();
	if(vehicle && vehicle->GetEntity() && vehicle->GetEntity()->GetClass()==m_pAAA)
	{
		fCompass = vehicle->GetEntity()->GetWorldAngles().z*180.0f/gf_PI;
		fCompass = g_pGame->GetHUD()->GetVehicleInterface()->GetRelativeHeading();
		fCompass -= 90.0f;
	}
	else
	{
		fCompass = actor->GetAngles().z*180.0f/gf_PI - 90.0f;
	}
...
void CHUDRadar::UpdateCompassStealth(CActor *pActor, float fDeltaTime)
{
	...
	float fCompass = GetRadarRotation(pActor);
...
void CHUDRadar::RenderMapOverlay()
{
	...
	if(pActor)
	{
		if(!pActor->GetLinkedVehicle())
		{
			...
			vPlayerPos.x = fX;
			vPlayerPos.y = fY;
			numOfValues += FillUpDoubleArray(&entityValues, pActor->GetEntity()->GetId(), EPlayer, fX, fY, 270.0f-RAD2DEG(pActor->GetEntity()->GetWorldAngles().z), ESelf, 100, 100, iOnScreenObjective==pActor->GetEntity()->GetId(), iCurrentSpawnPoint==pActor->GetEntity()->GetId());
		}
	}
	...
	for(int iEntity=0; iEntity<query.nCount; iEntity++)
	{
		...
		if(pEntity && !pEntity->IsHidden() && pEntity != pActorEntity)
		{
			...
			if(g_pGame->GetIGameFramework()->GetIItemSystem()->GetItem(id))
			{
				...
				if(pClientPlayer && !pClientPlayer->GetLinkedVehicle())
					pClientPlayer->AutoPickUpItem(id); //when this is enabled the player looks for items to pick up automatically (ammo for current weapon)
			}
			else
