...
CHUD::~CHUD()
{
  ...
  if(pPlayer)
  {
	...
	if(pLocalItem)
	{
		IWeapon *pLastWeapon = pLocalItem->GetIWeapon();
		if(pLastWeapon)
		{
			pLastWeapon->RemoveEventListener(this);
		}
	}
...
CWeapon * CHUD::GetCurrentWeapon()
{
	...
	IItem *item = actor->GetCurrentItem();
	if(!item)
		return 0;

	IWeapon *pWeapon = item->GetIWeapon();
	if(!pWeapon)
		return 0;
...
bool CHUD::Init()
{
	...
	{
		m_pHUDPowerStruggle = NULL;
		m_pHUDTeamInstantAction = NULL;
		m_pHUDInstantAction = NULL;
	}
	...
	{
		g_pGame->GetGameRules()->AddGameRulesListener(this);
	...
	m_hudFireModes["MOAR"] = 15; //Moac, Moar
	m_hudFireModes["MOAC"] = 14; //Moac, Moar
	...
	m_hudFireModes["AVMachinegun"] = 22;
	m_hudFireModes["AHMachinegun"] = 22;

	// Expanxion
	m_hudFireModes["Launch"] = 31;
	m_hudFireModes["LaunchBounce"] = 26;
	m_hudFireModes["LaunchRemote"] = 29;
	m_hudFireModes["LaunchEMP"] = 28;
	m_hudFireModes["LaunchBounceEMP"] = 27;
	m_hudFireModes["LaunchRemoteEMP"] = 30;
	...
	m_hudAmmunition["fgl40fraggrenade"] = 6;
	m_hudAmmunition["fgl40empgrenade"] = 18;
...
void CHUD::ResetPostSerElements()
{
	...
	if(m_animBuyMenu.GetVisible())
		ShowPDA(false, true);
	if(m_animTeamSelection.GetVisible())
		ShowPDA(false);
...
void CHUD::OnSetActorItem(IActor *pActor, IItem *pItem )
{
	...
	CWeapon *pCurrent = GetCurrentWeapon();
	if(!pCurrent || pCurrent->GetEntityId()!=m_uiWeapondID)
	{
		//weapon id has changed or current weapon is invalid, unregister/register
		IItem *pLocalItem = gEnv->pGame->GetIGameFramework()->GetIItemSystem()->GetItem(m_uiWeapondID);
		if(pLocalItem)
		{
			IWeapon *pLastWeapon = pLocalItem->GetIWeapon();
		...
		if(pItem)
		{
			if(pCurrent)
			{
				m_uiWeapondID = pCurrent->GetEntityId();
				IWeapon *pCurrentWeapon = pCurrent->GetIWeapon();
				if(pCurrentWeapon)
				{
					pCurrentWeapon->AddEventListener(this,__FUNCTION__);
				}
			}
			else
			{
				m_uiWeapondID = pItem->GetEntityId();
				IWeapon *pCurrentWeapon = pItem->GetIWeapon();
				if(pCurrentWeapon)
				{
					pCurrentWeapon->AddEventListener(this,__FUNCTION__);
				}
	...
	if(pItem && m_pHUDPowerStruggle && pGameRules)
	{
		...
		else if(m_uiWeapondID && (bIsInList || IsBuyMenuActive()))
		{
			...
			CWeapon *pLastWeapon = pLastWeaponItem ? static_cast<CWeapon *>(pLastWeaponItem) : NULL;
			const char *szName = pItem->GetEntity()->GetClass()->GetName();
			if(pLastWeapon && NULL == pLastWeapon->GetAccessory(szName))
			{
  ...
  if(g_pGameCVars->g_debug_fscommand)
	...
	if(!strcmp(szCommand,"WeaponAccessory"))
	{
		...
		if(pCurrentWeapon)
		{
			...
			CPlayer *pPlayer = static_cast<CPlayer *>(gEnv->pGame->GetIGameFramework()->GetClientActor());
			if (gEnv->pAISystem && pPlayer && pPlayer->GetEntity() && pPlayer->GetEntity()->GetAI() )
			{
				IAISignalExtraData* pData = gEnv->pAISystem->CreateSignalExtraData();//AI System will be the owner of this data
...
/*	else if(!stricmp(szCommand,"soundstop_malfunction"))
	...
	else if(!stricmp(szCommand,"buymenu_step_down"))
	{
		//key buying next layer
		ChangeBuyMenuLayer((SBuyMenuKeyLog::EBMKL_State)(((int)m_buyMenuKeyLog.m_state)+1));
	}
	else if(!stricmp(szCommand,"buymenu_step_up"))
	{
		//key buying previous layer
		ChangeBuyMenuLayer((SBuyMenuKeyLog::EBMKL_State)(((int)m_buyMenuKeyLog.m_state)-1));
	}
	else if(!stricmp(szCommand,"buymenu_step_reset"))
	{
		//key buying first layer
		ChangeBuyMenuLayer(SBuyMenuKeyLog::eBMKL_First);
	}
	else if(!stricmp(szCommand,"buymenu_step_clear"))
	{
		//key buying no layer
...
bool CHUD::ShowPDA(bool show, bool buyMenu)
{
	...
	if( buyMenu && show )
		if ( EntityId currentItem = pActor->GetCurrentItemId(true) )
	...
	if (show && m_pModalHUD == NULL)
	{
		...
		if(!buyMenu)
		{
			...
			CPlayer *pPlayer = static_cast<CPlayer *>(pActor);
			anim->SetVariable("SpectatorMode",SFlashVarValue((pActor->GetSpectatorMode() != CActor::eASM_None) && pGameRules->GetTeam(pActor->GetEntityId()) == 0));
		...
		if(buyMenu)
		{
			...
			m_pHUDPowerStruggle->DetermineCurrentBuyZone(true, true);
		}
...
void CHUD::OnHardwareMouseEvent(int iX,int iY,EHARDWAREMOUSEEVENT eHardwareMouseEvent)
{
	...
	{
		int x(iX), y(iY);
		if(eCursorState==SFlashCursorEvent::eCursorMoved)
		{
			m_animPDA.GetFlashPlayer()->ScreenToClient(x,y);
			m_animPDA.GetFlashPlayer()->SendCursorEvent(SFlashCursorEvent(eCursorState,x,y));
		}
...
void CHUD::GetGPSPosition(wchar_t *szN,wchar_t *szW)
{
	...
	if(pLocMgr)
	{
		pLocMgr->LocalizeLabel("@ui_radar_N", strNorth);
		pLocMgr->LocalizeLabel("@ui_radar_W", strWest);
	}
...
void CHUD::OnPostUpdate(float frameTime)
{
	...
	if(m_pHUDPowerStruggle)
	{
		...
		{
			int diff = pp - m_lastPlayerPPSet;

			m_animPlayerPPChange.Invoke("addLog",diff);
			m_animPlayerPP.Invoke("setPPoints", pp);
			m_lastPlayerPPSet = pp;
			m_pHUDPowerStruggle->ObjectiveStatusChanged();
		}
		...
		if(m_animScoreBoard.IsLoaded())
		{
			...
			if(gEnv->bMultiplayer)
			{
				if (CGameRules *pGameRules=g_pGame->GetGameRules())
				{
					...
					{
						int timelimit = g_pGameCVars->g_timelimit;
						if(timelimit>0)
						{
							int minutes=time/60;
							int seconds=time-(minutes*60);
						...
						{
							m_animScoreBoard.Invoke("setCountdown", "99:99");
						}
					}
				}
			}
...
void CHUD::SetTeamDisplay(const char* team)
{
	if(m_animHexIcons.IsLoaded())
	{
		...
		{
			m_animHexIcons.Invoke("setFlagIcon", team);
...
bool CHUD::UpdateTimers(float frameTime)
{
	...
	{
		m_fFlashTimer -= frameTime;
		if(m_fFlashTimer>(m_fFlashTime*m_fBlindTime))
		{
			m_animMissionObjective.SetVariable("_alpha",0.0f);
		}
		else if(m_fFlashTimer>0)
		{
			float alpha = 100.0f - (m_fFlashTimer*100.0f)/(m_fFlashTime*m_fBlindTime);
			m_animMissionObjective.SetVariable("_alpha",alpha);
		}
		if(m_fFlashTimer<=0.0f && m_fFlashTimer!=-1.0f)
		{
			m_fFlashTimer = -1.0f;
			m_animMissionObjective.SetVariable("_alpha",100.0f);
		}
	}
	...
	if(m_fSuitChangeSoundTimer != 0)
	{
		...
		{
			if(pPlayer && pPlayer->GetNanoSuit() && pPlayer->GetHealth()>0 && pPlayer->GetSpectatorMode() == CActor::eASM_None)
			{
...
void CHUD::SetFireMode(IItem *pItem, IFireMode *pFM, bool forceUpdate)
{
	...
	if(iFireMode == 0 || iFireMode == 9) //Shotgun has the firemode "Shotgun"
	{
		if(pFM)
		{
			...
			if(pFM->GetAmmoType())
			{
				if (!strcmp(pFM->GetAmmoType()->GetName(), "incendiarybullet"))
				{
					if(iFireMode == 1)
						iFireMode = 18;
					else if(iFireMode == 3)
				...
				else if (!strcmp(pFM->GetAmmoType()->GetName(), "fgl40empgrenade"))
				{
					switch (iFireMode)
					{
						case 31:
			...
			if(pFM->GetClipSize() == -1 && iFireMode != 6 && iFireMode != 14 && iFireMode != 15)
				iFireMode = 8;
		}
	}
...
void CHUD::AddToScoreBoard(EntityId player, int kills, int deaths, int ping, int teamKills)
{
	if(m_pHUDScore)
		m_pHUDScore->AddEntry(player, kills, deaths, ping, teamKills);
}
...
void CHUD::ChangeBuyMenuLayer(SBuyMenuKeyLog::EBMKL_State state)
{
	if(state==SBuyMenuKeyLog::eBMKL_First || state==SBuyMenuKeyLog::eBMKL_Last)
	{
		state = SBuyMenuKeyLog::eBMKL_Tab;
	}
	m_buyMenuKeyLog.m_state = state;
	m_animBuyMenu.Invoke("activateNumbers",SFlashVarValue((int)state));
}
...
bool CHUD::ShowWeaponAccessories(bool enable)
{
	...
	if (enable)
	{
		...
		bool allowZoom = true;
		if(pWeapon->GetFireMode(pWeapon->GetCurrentFireMode()))
			allowZoom = pWeapon->GetFireMode(pWeapon->GetCurrentFireMode())->AllowZoom();

		if(m_acceptNextWeaponCommand && !busy && !reloading && !modify && !zooming && !switchFM && !sprinting && !isTargeting && allowZoom)
		{
...
void CHUD::GameOver(int localWinner)
{
	if(!m_animScoreBoard.IsLoaded()) return;

	//if(g_pGame && g_pGame->GetGameRules() && g_pGame->GetGameRules()->GetTeamCount() <= 1) return;
	...
	CryFixedStringT<64> message("@ui_msg_draw_0");
	...
	localizedText = LocalizeWithParams(message, false);

	SFlashVarValue args[2] = {localizedText, false};
	m_animMPMessages.Invoke("addKillLog", args, 2);
	m_animScoreBoard.Invoke("setWinText", message.c_str());
}
