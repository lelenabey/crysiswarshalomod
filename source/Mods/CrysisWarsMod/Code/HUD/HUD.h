...
public:
	...
	{
		SOnScreenIcon(EntityId _id, int _x, int _y, int _icontype, int _friendly, float _distance, float _size, float _rotation) : id(_id), x(_x), y(_y), icontype(_icontype), friendly(_friendly), distance(_distance), size(_size), rotation(_rotation) {}
		EntityId id;
		int x;
		int y;
		int icontype;
  ...
  void SetInMenu(bool m);
	...
	void DisplayGameStartMessage(const char* label, const char* seconds, const char* scoreLimit, const char* timeLimit);
