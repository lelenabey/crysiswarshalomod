...
#define SCRIPT_REG_CLASSNAME &CScriptBind_HUD::
	...
	SCRIPT_REG_TEMPLFUNC(ResetBuyZones, "");
	...
	SCRIPT_REG_TEMPLFUNC(DisplayKillMessage, "name, times, teamkill, frag, points");
	...
	if(usable == 1)
	{
		if(IVehicle *pVehicle = gEnv->pGame->GetIGameFramework()->GetIVehicleSystem()->GetVehicle(objId))
		{
			...
			if(IActor *pActor = g_pGame->GetIGameFramework()->GetClientActor())
			{
				...
				if(usable!=2 && pVehicle->GetEntity())
				{
					bool result=true;

					if (IScriptTable *pScriptTable=pVehicle->GetEntity()->GetScriptTable())
					{
						HSCRIPTFUNCTION pfnCanEnter=0;
						if (pScriptTable->GetValue("CanEnter", pfnCanEnter) && pfnCanEnter)
						{
							Script::CallReturn(gEnv->pScriptSystem, pfnCanEnter, pScriptTable, ScriptHandle(pActor->GetEntityId()), result);
							gEnv->pScriptSystem->ReleaseFunc(pfnCanEnter);
						}
					}
					if(!result)
					{
						usable = 2;
						textLabel = "@use_vehicle_reserved";
					}
				}
			}
		}
...
int CScriptBind_HUD::ResetBuyZones(IFunctionHandler *pH)
{
	// call the hud, tell him to reset the stored buy zones
	CHUD *pHUD = g_pGame->GetHUD();
	if (pHUD && pHUD->GetPowerStruggleHUD())
		pHUD->GetPowerStruggleHUD()->ResetBuyZones();
...
int CScriptBind_HUD::OnItemBought(IFunctionHandler *pH, bool success, const char* itemName)
{
	...
	if(success && itemName && pHUD->GetPowerStruggleHUD())
	{
		...
		IEntityClass* pClass = gEnv->pEntitySystem->GetClassRegistry()->FindClass(itemName);
		CAmmoPickup::ShouldSwitchGrenade(pClass, pActor);
	}
...
int CScriptBind_HUD::DisplayKillMessage(IFunctionHandler *pH, const char* name, int times, bool teamkill, bool frag, bool selfkill, int points)
{
	if (CHUD *pHUD = g_pGame->GetHUD())
	{
		pHUD->DisplayKillMessage(name, times, teamkill, frag, selfkill, points);
	}
	return pH->EndFunction();
}
