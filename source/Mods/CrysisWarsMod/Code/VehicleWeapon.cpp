...
void CVehicleWeapon::StartFire()
{
  ...
  CWeapon::StartFire();
	...
	// vehicle weapons don't 'reload' as such, since ammo comes directly from 
	//	inventory. However for the tank cannons we want a reload sound at this point,
	//	so we play the action manually. (Only tanks have the reload action currently).
	if(m_fm->GetNextShotTime() > 0.01f)
	{	
		PlayAction("reload");			
	}	
}
