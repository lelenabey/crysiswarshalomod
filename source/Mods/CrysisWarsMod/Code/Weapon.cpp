...
CWeapon::~CWeapon()
{
	...
	if( pOwner )
		pOwner->SetDropWeaponTimer(GetEntityId(), -1.f);
	...
	if(m_params.raiseable)
	{
		if(raise && !m_weaponRaised)
		{
			...
			if ((pPlayer != NULL) && pPlayer->IsClient())
			{
				...
				if(IsBusy() || IsModifying() || (m_fm && (m_fm->IsFiring()||m_fm->IsReloading())))
					return;
...
//-------------------------------------------------------
void CWeapon::AutoDrop(bool doCount)
{
	if(m_fm)
	{
		if(doCount) 
		...
		if(OutOfAmmo(false) && (GetAmmoCount(m_fm->GetAmmoType())<=0) && (m_firedRockets<=0 || !doCount))
		{
	...
	if(m_fm)
	{
		...
		if( pOwner )
		{
			...
			else
			{
				pOwner->DropItem(GetEntityId(),2.0f,false,false);
				if(IsServer())
					pOwner->GetGameObject()->InvokeRMI(CActor::ClSelect(), CActor::NoParams(), eRMI_ToClientChannel,pOwner->GetGameObject()->GetChannelId());
			}
		}
		if(!gEnv->bMultiplayer)
		{
			...
			if( pGameRules )
				pGameRules->ScheduleEntityRemoval(GetEntityId(),5.0f,true);
		}
	}
}
		...
		if(pInventory)
		{
			...
			if(!m_ammo.empty())
			{
				for (TAmmoMap::const_iterator it=m_ammo.begin(); it!=m_ammo.end(); ++it)
				{
					int invAmmo  = pInventory->GetAmmoCount(it->first);
					int invLimit = pInventory->GetAmmoCapacity(it->first);
					//kirill - let's allow to pick-up if any ammo is not filled up to limit
...
//------------------------------------------------------------------------
bool CWeapon::CanUseAmmo(IEntityClass* pAmmoType)
{
	TAmmoMap::const_iterator it = m_ammo.find(pAmmoType);
	if (it != m_ammo.end())
		return true;
...
float CWeapon::GetRecoilAmount() const
{
	if (m_fm)
	{
		return m_fm->GetRecoilAmount();
	}
