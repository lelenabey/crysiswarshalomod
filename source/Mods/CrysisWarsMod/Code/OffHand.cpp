			...
			if (CHUD* pHUD = g_pGame->GetHUD())
			{
				...
				else if(pItem)
				{
					...
					if(pItem->GetIWeapon())
					{
						...
						bool isSocom = (strcmp(pItemClass->GetName(),"SOCOM") && strcmp(pItemClass->GetName(),"AY69")) ? false:true;
						...
						else
						{
							...
							{
								IItem *pExchangedItem = GetExchangeItem(m_pItemSystem->GetItem(m_preHeldEntityId), pPlayer);
								if(pExchangedItem)
								{
		...
		case eOHA_USE:						
			{
															...
															// EXP1: Check dual wield weaponry for inverse mechanism
															CItem *pMain = GetActorItem(GetOwnerActor());
															bool invert = pMain ? pMain->IsDualWield() : false;

															if(inputMethod==INPUT_LBM)
															{
																//Two handed object throw/drop is handled with the mouse now...
																if(activationMode==eAAM_OnPress && m_currentState&eOHS_HOLDING_OBJECT && (m_grabType==GRAB_TYPE_TWO_HANDED || invert))
																{
																	m_forceThrow = !invert;
																	return true;
																}
																else if(activationMode==eAAM_OnRelease && m_currentState&eOHS_THROWING_OBJECT && (m_grabType==GRAB_TYPE_TWO_HANDED || invert))
																{
																	return true;
																}
															}
															...
															{
																//Two handed object throw/drop is handled with the mouse now...
																if(activationMode==eAAM_OnPress && m_currentState&eOHS_HOLDING_OBJECT && !invert)
																{
																	m_forceThrow = false;
																	return true;
																}
																else if(activationMode==eAAM_OnRelease && m_currentState&eOHS_THROWING_OBJECT && !invert)
																{
																	return true;
																}
															}
...
bool COffHand::PreExecuteAction(int requestedAction, int activationMode, bool forceSelect)
{
	...
	if(exec)
	{
		...
		{
			if (requestedAction != eOHA_REINIT_WEAPON)
...
int COffHand::CanPerformPickUp(CActor *pActor, IPhysicalEntity *pPhysicalEntity /*=NULL*/, bool getEntityInfo /*= false*/)
{
	...
	if (!pPhysicalEntity)
	{
		...
		{
			int preHeldTemp=m_preHeldEntityId;
			if(CheckItemsInProximity(info.eyePosition,info.eyeDirection, true)==OH_GRAB_ITEM)
			{
				int grab=OH_NO_GRAB; // height diff check for grab (CheckItemsInProximity() use 2D entity search)
				if(cry_fabsf(info.eyePosition.z-m_pEntitySystem->GetEntity(m_preHeldEntityId)->GetWorldPos().z)<3.f)
					grab=OH_GRAB_ITEM;
				if(!getEntityInfo)						
					m_preHeldEntityId=preHeldTemp;
				return grab;
			}
			else
				return OH_NO_GRAB;
		...
		if (CItem *pItem=static_cast<CItem*>(m_pItemSystem->GetItem(id)))
		{
			if (pItem->GetOwnerId()!=GetOwnerId())
			{
				...
				if (dir.Dot(itemPos-pos)>0.0f)
				{
					...
					// SNH: added extra z test here as QueryProximity ignores z
					if(dstSqr<0.2f && abs(itemPos.z - pos.z) < (sizeZup + sizeZdown))
					{
...
{	
	...
	if(!pPlayer->CheckInventoryRestrictions(pPreHeldEntity->GetClass()->GetName()))
	{
		...
		IItem* pPickupItem =m_pItemSystem->GetItem(m_preHeldEntityId);
		IItem* pItem = GetExchangeItem(pPickupItem, pPlayer);
		if(pItem)
		{
			bool dual_drop=false;
			bool dual_success=false;
			EntityId dual=0;
			dual=pItem->GetDualWieldMasterId();
			if (dual == 0)
				dual=pItem->GetDualWieldSlaveId();

			if(pPlayer->DropItem(pItem->GetEntityId(), 8.f, true))
				drop_success=true;

			if (dual!=0)
			{
				IItem* pDual = GetExchangeItem(pPickupItem, pPlayer);
				if (pDual && pPlayer->DropItem(pDual->GetEntityId(), 30.f, true))
					dual_success=true;
			}

			drop_success=drop_success && (dual_drop ? dual_success : true);
		}
...
int COffHand::CanExchangeWeapons(IItem *pItem, IItem **pExchangeItem)
{
	...
	{
		//Can not carry more heavy/medium weapons
		IItem* pXCItem = GetExchangeItem(pItem, pPlayer);
		if(pXCItem)
		{
			*pExchangeItem = pXCItem;
...
IItem* COffHand::GetExchangeItem(IItem *pPickupItem, CPlayer *pPlayer)
{
	...
	const char *pickupCategory = NULL;
	if(pPickupItem && pItem)
	{
		itemCategory = m_pItemSystem->GetItemCategory(pItem->GetEntity()->GetClass()->GetName());
		pickupCategory = m_pItemSystem->GetItemCategory(pPickupItem->GetEntity()->GetClass()->GetName());
		...
		if (!strcmp(itemCategory, pickupCategory) && pItem->CanDrop())
			return pItem;
		else
		{
			int i = pPlayer->GetInventory()->GetCount();
			for(int w = i-1; w > -1; --w)
			{
				pItem = g_pGame->GetIGameFramework()->GetIItemSystem()->GetItem(pPlayer->GetInventory()->GetItem(w));
				itemCategory = m_pItemSystem->GetItemCategory(pItem->GetEntity()->GetClass()->GetName());
				if (!strcmp(itemCategory, pickupCategory) && pItem->CanDrop() && !pItem->IsDualWieldSlave())
					return pItem;
			}
		}
	}
		...
		if(pAttachment)
		{
			...
			else
			{
				...
				case 1:
					...
					break;
				case 2:
					slot = eIGS_OwnerLooped;	// yes, this is correct
					break;
				case 3:
					slot = eIGS_Owner;				// so is this
					break;
				default:
					break;
				}
