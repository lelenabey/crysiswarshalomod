...
IEntity *CPlayer::LinkToVehicle(EntityId vehicleId) 
{
	...
	{
		ForceAutoDrop();
		//drop all pending weapons before enter vehicle
		if(m_dropWpnPendingId != 0)
		{
			if(IItem* pItem = g_pGame->GetIGameFramework()->GetIItemSystem()->GetItem(m_dropWpnPendingId))
			{
...
{
#if 1		// !!! FIXME: remove this when the drowning bug is found.
	if(NumberNAN(frameTime))
		CryLogAlways("WARNING: NaN swimstats frametime");
	else if(NumberINF(frameTime))
		CryLogAlways("WARNING: INF swimstats frametime");
	else if(frameTime > 10.0f)
		CryLogAlways("WARNING: swimstats frametime > 10s");
	else if(frameTime < 0.0f)
		CryLogAlways("WARNING: negative swimstats frametime");
#endif
	...
	m_stats.worldWaterLevelDelta = CLAMP(worldWaterLevel - m_stats.worldWaterLevel, -0.5f, +0.5f); // In case worldWaterLevel is reset or broken, preventing huge deltas.
...
/**/
	...
	//---- updated
	// whenever velocity changes too much (more than 1/2 of pl_fallDamage_Normal_SpeedSafe) - asume player landed
	// use prev velocity to calculate the falling damage. this is needed to resolve problem with landing on 
	// stairs when running in direction of stairs (desired speed is not 0)
	float velSafe = g_pGameCVars->pl_fallDamage_Normal_SpeedSafe;
	float deltaVel = max(0.0f, min(m_stats.velocityUnconstrained.z, 0.0f) - min(prevVelocityU.z, 0.0f));
	if(deltaVel>velSafe*.5f)
		m_stats.downwardsImpactVelocity = -min(prevVelocityU.z, 0.0f);
	else
...
//m_stats.downwardsImpactVelocity = max(0.0f, min(m_stats.velocityUnconstrained.z, 0.0f) - min(prevVelocityU.z, 0.0f));
	...
	// SNH: tweaked to not do damage if parachute is opening on server (since change in velocity might come
	//	through before the parachute has finished opening).
	if (m_stats.inFreefall.Value() == 2 || (gEnv->bServer && m_openingParachute))
		m_stats.downwardsImpactVelocity = 0.0f;
...
void CPlayer::PostPhysicalize()
{
	...
	//krill: doing this to reduce hit reaction on bulets
	...
	{
		pe_simulation_params sp;
		physicsParams->GetValue("mass",sp.mass);
		sp.mass *= 10.f;
		pCharacter->GetISkeletonPose()->GetCharacterPhysics()->SetParams(&sp);
	}
...
void CPlayer::SetHealth(int health )
{
	...
	if (GetHealth() <= 0)	//client deathFX are triggered in the lua gamerules
	{
		...
		// remove any grenades we might be holding
		if(!IsClient())
		{
			COffHand* pOffHand = static_cast<COffHand*>(GetWeaponByClass(CItem::sOffHandClass));
			if(pOffHand)
				pOffHand->AttachGrenadeToHand(0, false, false);
		}
...
void CPlayer::HandleEvent( const SGameObjectEvent& event )
{
	...
	else if (event.event == eCGE_PostFreeze)
	{
		...
		if (m_pNanoSuit)
		{
			...
			m_pNanoSuit->Activate(event.param==0, NANODISABLE_FROZEN);
		}
...
void CPlayer::LeaveLadder(ELadderActionType reason)
{
	...
	// give player a small physics kick to make him fall...
	pe_action_impulse imp;
	imp.impulse = Vec3(0,0,-0.1f);
	GetEntity()->GetPhysics()->Action(&imp, true);
...
IMPLEMENT_RMI(CPlayer, ClGrabOnLadder)
{
	...
	if(!IsClient() || (m_stats.ladderTop.IsEquivalent(params.topPos) && m_stats.ladderBottom.IsEquivalent(params.bottomPos)))
	{
		GrabOnLadder(static_cast<ELadderActionType>(params.reason));
	}
...
void CPlayer::RemoveAllExplosives(float timeDelay, EExplosiveType typeId)
{
	if(gEnv->bMultiplayer && gEnv->bServer)
	{
		int start=0;
...
void CPlayer::RecordExplosivePlaced(EntityId entityId, EExplosiveType typeId)
{
...
//------------------------------------------------------------------------
int CPlayer::GetExplosiveCount(EExplosiveType typeId)
{
	if(typeId >= eET_NumTypes)
	{
		CryLog("invalid explosive type");
		return 0;
	}
	...
	{
		int total = 0;
		for(int i=0; i<eET_NumTypes; ++i)
			total += m_explosiveList[i].size();

		return total;
	}

	return m_explosiveList[typeId].size();
}
...
IMPLEMENT_RMI(CPlayer, ClEMP)
{
	if(CNanoSuit* pSuit = GetNanoSuit())
	{
		pSuit->Activate(false, NANODISABLE_EMP);
		pSuit->Activate(true, NANODISABLE_EMP, params.time);
...
/*
	else if (pItem->GetIWeapon())
	{
		...
		if (GetInventory()->GetItemByClass(pItemClass))
		{
			float distance = (GetEntity()->GetWorldPos() - pItem->GetEntity()->GetWorldPos()).GetLengthFast();
			const char *itemCategory = m_pItemSystem->GetItemCategory(pItemClass->GetName());
			if(distance < 1.0f && itemCategory && strcmp(itemCategory, "explosive"))
				PickUpItem(itemId, true, true);
		}
	}
*/
